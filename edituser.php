            ...
            if ($uploader->upload()) {
                ...
                if (!$avatarHandler->insert($avatar)) {
                    ...
                    if (!empty($oldavatar) && false !== strpos(strtolower($oldavatar), 'cavt')) {
                        $avatars = $avatarHandler->getObjects(new Criteria('avatar_file', $oldavatar));
                        if (!empty($avatars) && count($avatars) == 1 && is_object($avatars[0])) {
                            $avatarHandler->delete($avatars[0]);
                    ...
                    $avatarHandler->addUser($avatar->getVar('avatar_id'), $GLOBALS['xoopsUser']->getVar('uid'));
...
if ($op === 'avatarchoose') {
    ...
    if (!empty($_POST['user_avatar'])) {
        ...
        $avatars = $avatarHandler->getObjects($criteria_avatar);
    ...
    if (0 === strpos($user_avatarpath, realpath(XOOPS_UPLOAD_PATH)) && is_file($user_avatarpath)) {
        ...
        if ($user_avatar !== 'avatars/blank.gif') {
            $avatars = $avatarHandler->getObjects(new Criteria('avatar_file', $user_avatar));
            if (is_object($avatars[0])) {
                $avatarHandler->addUser($avatars[0]->getVar('avatar_id'), $GLOBALS['xoopsUser']->getVar('uid'));
            }
        }
    }
